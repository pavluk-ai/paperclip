...
describe("codex execute", () => {
  ...
  it("refreshes managed config.toml from the shared Codex home on later runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "paperclip-codex-execute-refresh-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      const firstResult = await runOnce("run-refresh-1");
      ...
      const secondResult = await runOnce("run-refresh-2");
