...
export function stripMonitorFromExecutionPolicy(policy: IssueExecutionPolicy | null): IssueExecutionPolicy | null {
  ...
  const strippedPolicy: IssueExecutionPolicy = {
    mode: policy.mode,
    followUpMode: policy.followUpMode ?? null,
    commentRequired: policy.commentRequired,
    stages: policy.stages,
  };
  if (policy.stages.length === 0 && policy.mode !== "checkpoint" && !policy.followUpMode) return null;
