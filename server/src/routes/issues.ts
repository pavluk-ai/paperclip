...
export function issueRoutes(
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    void (async () => {
      ...
      const hasWakeupForIssue = (agentId: string, wakeIssueId: string) => wakeups.has(`${agentId}:${wakeIssueId}`);
      ...
      if (sameAssigneeManagerHandoff && issue.assigneeAgentId && comment && !hasWakeupForIssue(issue.assigneeAgentId, issue.id)) {
        addWakeup(issue.assigneeAgentId, {
          source: "automation",
          triggerDetail: "system",
      ...
      if (issue.parentId && childEnteredParentReconciliationState) {
        try {
          ...
          if (
            ...
            !hasWakeupForIssue(parentIssue.assigneeAgentId, parentIssue.id)
          ) {
            addWakeup(parentIssue.assigneeAgentId, {
              source: "automation",
              triggerDetail: "system",
      ...
      if (commentBody && comment) {
        ...
        const scheduledRetryCommentWakeEligible =
          scheduledRetrySupersededByComment && !isCheckpointIssue;
        const commentOnlyWakeEligible =
          !assigneeChanged &&
          !statusChanged &&
        ...
        if (
          ...
          (
            reopened ||
            ((scheduledRetryCommentWakeEligible || commentOnlyWakeEligible) && !skipAssigneeCommentWake)
          )
        ) {
