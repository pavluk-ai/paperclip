    ...
    onboarding: {
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Runtime-specific adapter config. OpenClaw Gateway agents must include url (ws:// or wss://), headers.x-openclaw-token (or legacy x-openclaw-auth), and should include agentId for persona-bound routing. Optional OpenClaw fields: paperclipApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, payloadTemplate, role, scopes, disableDeviceAuth, devicePrivateKeyPem. Other runtimes should include the config their adapter expects."
      },
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    ...
    For OpenClaw Gateway, read the local gateway token and derive persona routing before submitting the join request.
    ...
    On successful claim, save the full JSON response somewhere private for your runtime.
    For OpenClaw Gateway, save the response to the current OpenClaw persona workspace as:
